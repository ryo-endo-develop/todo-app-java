...
/**
 * ドメイン層の例外基底クラス Effective Java: Item 70 (復旧可能な状況にはチェック例外を、プログラミングエラーには実行時例外を使う)
 * ビジネスルール違反やドメイン不整合を表現 重要な基盤クラスのため、fail-fastを重視してObjects.requireNonNullを使用
 */
@Getter
public class DomainException extends RuntimeException {
  ...
  /** ファクトリメソッド: ビジネスルール違反 */
  @Nonnull
  public static DomainException businessRuleViolation(
      @Nonnull String ruleName, @Nonnull String details) {
  ...
  public static DomainException invalidState(
      @Nonnull String currentState, @Nonnull String operation) {
    Objects.requireNonNull(currentState, "Current state must not be null");
    Objects.requireNonNull(operation, "Operation must not be null");
    return new DomainException(
        String.format("Invalid state [%s] for operation [%s]", currentState, operation));
  }
