 ...
 *
 * <p>設計原則: - Effective Java Item 17: 不変性を最大化 - Effective Java Item 69: 例外は例外的な状況でのみ使用 -
 * Thread-safe: 全フィールドがfinalで不変 - Fail-fast: 不正な状態を早期検出
 *
 * @param <T> 成功時の値の型
 */
@Immutable
...
public final class Result<T> {
  ...
  /** 成功時の値をOptionalで取得 Thread-safe: 状態は不変なので同期不要 */
  @Nonnull
  ...
  /**
   * 関数型スタイルでの値変換 Thread-safe: 新しいResultオブジェクトを返すため
   *
  ...
  /** 成功時にアクションを実行 Thread-safe: thisを返すが状態は変更しない */
  @Nonnull
  ...
  /** 等価性の判定 Result の状態（成功/失敗）と内容を正しく比較する */
  @Override
