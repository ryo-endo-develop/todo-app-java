...
/** ユーザー名を表現する値オブジェクト Effective Java: Item 62 (他の型が適切な場合は文字列を避ける) ビジネスルールとバリデーションを内包 */
@Getter
@EqualsAndHashCode
@ToString
public final class UserName {
  ...
  private static final Pattern VALID_CHARS =
      Pattern.compile("^[a-zA-Z0-9_\\-\\p{IsHiragana}\\p{IsKatakana}\\p{IsHan}]+$");
  ...
  /** 文字列からUserNameを作成 ビジネスルールによるバリデーション付き */
  public static Result<UserName> of(@Nonnull String value) {
    ...
    if (trimmed.length() > MAX_LENGTH) {
      return Result.failure(
          "ユーザー名は" + MAX_LENGTH + "文字以内で入力してください（現在: " + trimmed.length() + "文字）");
    }
