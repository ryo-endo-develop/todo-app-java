 ...
 *
 * <p>責務: - タイトル文字列の検証と正規化 - ビジネスルールの適用（長さ制限、文字種制限） - 検索・比較機能の提供
 *
 * <p>拡張性: - バリデーション戦略パターンの追加が容易 - 国際化対応の文字種制限変更が可能 - 新しいビジネスルール追加に対応
 */
@Immutable
@ThreadSafe
@Getter
...
public final class TodoTitle {
   ...
   * 文字列からTodoTitleを作成
   *
   * <p>処理フロー: 1. 基本的なnullチェック 2. 前後空白の削除（正規化） 3. 長さ制限の検証 4. 文字種制限の検証
   *
  ...
  public static Result<TodoTitle> of(@Nonnull String value) {
    ...
    if (characterValidation.isFailure()) {
      return Result.failure(
          characterValidation.getErrorMessage().orElse("Unknown validation error"));
    }

    return Result.success(new TodoTitle(normalized));
  }

  /** 長さ制限の検証 Strategy パターンで将来的なルール拡張に対応 */
  @Nonnull
  private static Result<Void> validateLength(@Nonnull String value) {
    ...
    if (value.length() < ValidationRules.MIN_LENGTH) {
      return Result.failure(String.format("Todoタイトルは%d文字以上で入力してください", ValidationRules.MIN_LENGTH));
    }

    if (value.length() > ValidationRules.MAX_LENGTH) {
      return Result.failure(
          String.format(
              "Todoタイトルは%d文字以内で入力してください（現在: %d文字）", ValidationRules.MAX_LENGTH, value.length()));
    }
  ...
  /**
   * タイトルが特定のキーワードを含むかどうかを判定 複数キーワードのAND検索
   *
