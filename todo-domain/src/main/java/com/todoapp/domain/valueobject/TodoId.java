 ...
 * TodoのIDを表現する値オブジェクト
 *
 * <p>設計原則: - Immutable: すべてのフィールドがfinal - Thread-safe: 状態が不変なので同期不要 - Fail-fast: 不正な値を早期検出 - Single
 * Responsibility: IDの表現と検証のみ
 *
 * <p>拡張性: - ファクトリメソッドパターンで生成方法を制御 - 将来的なID形式変更に対応可能 - バリデーション戦略の追加が容易
 */
@Immutable
@ThreadSafe
@Getter
...
public final class TodoId {
  ...
  public static Result<TodoId> of(@Nonnull String value) {
    ...
    try {
      ...
      return Result.failure(String.format("TodoId string must be a valid number: '%s'", value));
    }
  }
  ...
  /**
   * 指定されたIDより大きいかどうかを判定 ソート処理などで使用
   *
   * @param other 比較対象（non-null）
