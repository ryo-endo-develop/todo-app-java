...
public final class TodoStatusTransitionPolicy {
    ...
    private void initializeDefaultRules() {
        // TODO状態からの遷移
        transitionRules.put(TodoStatus.TODO, Set.of(
        ...
        // IN_PROGRESS状態からの遷移
        transitionRules.put(TodoStatus.IN_PROGRESS, Set.of(
            TodoStatus.TODO,
            TodoStatus.COMPLETED,
            TodoStatus.DELETED
        ));
        ...
        // COMPLETED状態からの遷移
        transitionRules.put(TodoStatus.COMPLETED, Set.of(
            TodoStatus.TODO,
            TodoStatus.IN_PROGRESS,
            TodoStatus.DELETED
        ));
    ...
    /**
     * 批准ワークフローのような複雑な遷移ルール判定
     * ビジネスロジックをサービス層に集約
     */
    public boolean canTransitionWithApproval(@Nonnull TodoStatus currentStatus, 
                                           @Nonnull TodoStatus newStatus,
                                           boolean hasApproval) {
        // 基本的な遷移チェック
     ...
     * 緊急時の特別遷移ルール
     */
    public boolean canEmergencyTransition(@Nonnull TodoStatus currentStatus, 
                                        @Nonnull TodoStatus newStatus,
                                        boolean isEmergency) {
        if (!isEmergency) {
